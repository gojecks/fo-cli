...
const { prompt, validate } = require('../prompt');
...
class Apps {
    ...
    static async check_and_update(orgAndApp) {
    ...
    static async list(orgAndApp) {
        ...
        const details = appNames.map(name => `> ${name}  - ${appStorage[apps[name].storage] || 'F3 Custom'} - Version<${apps[name].version}> - Envs <${apps[name]?.metadata?.deployments.length || 0}>`);
        console.log('\x1b[33m%s\x1b[0m', details.join('\n'));
    }
}
