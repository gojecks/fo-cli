...
class Sites {
    ...
    static async deploy(orgAndApp) {
        ...
        const handleResponse = res => {
            ...
            if (answers.compress) {
                // cleanup by removing the compressed file
                fs.unlinkSync(answers.filePath);
            }
        };
    ...
    static async rename(orgAndApp, newName) {
        const name = await promptSiteName(orgAndApp);
        const sitesObject = getSitesInfo(orgAndApp);
     ...
     * @param {*} appName 
     */
    static async push_config(orgAndApp) {
        const name = await promptSiteName(orgAndApp);
        const sitesObject = getSitesInfo(orgAndApp);
    ...
    static async drop_artefact(orgAndApp) {
